...
try:
    ...
    raise unittest.SkipTest("Cannot import ESS modules.")
...
class ConfigTestCase(salobj.BaseConfigTestCase, unittest.TestCase):
    ...
    def test_ESS_DataClient_config(self):
        """Validate the ESS DataClient configuration.

        The ESS CSC itself only defines the general configuration schema. That
        ...
        config_dir_name = self.get_config_dir(
            ...
            sal_name=sal_name,
            schema=csc.CONFIG_SCHEMA,
        )
        config_dir = pathlib.Path(config_dir_name)
        config_files = list(config_dir.glob("*.yaml"))
        assert len(config_files) == 1, "Expected only one ESS configuration file."

        # Validate that the expected config file exists.
        config_file = config_dir / "_init.yaml"
        assert config_file.exists(), "Didn't find ESS config file _init.yaml."

        # Load the config.
        config = ConfigurableCsc.read_config_files(
            config_validator=StandardValidator(csc.CONFIG_SCHEMA),
